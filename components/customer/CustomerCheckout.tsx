  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-4 py-6 sm:px-6">
      <header className="flex items-center gap-4 border-b-2 border-[#dfe6e1] pb-5">
        <Link href={cart.storeId ? `/customer/stores/${cart.storeId}` as never : "/customer/stores" as never} className="grid size-11 place-items-center rounded-full border-2 border-[#cbd8cf] bg-white" aria-label="Back to products"><ArrowLeft size={22} /></Link>
      ...
      {cart.items.length === 0 ? (
        <section className="mt-10 rounded-3xl border-2 border-[#dce5df] bg-white p-10 text-center"><ShoppingBag className="mx-auto text-[#128646]" size={54} /><h2 className="mt-4 text-2xl font-black">Your list is empty</h2><p className="mt-2 text-[#65736a]">Add real products from a GreenChoice store to see them here.</p><Link href={"/customer/stores" as never} className="mt-6 inline-flex rounded-xl bg-[#087c39] px-6 py-3 font-black text-white">Browse stores</Link></section>
      ) : (
