export function ProductImage({ slug, name, src, large = false }: { slug: string; name: string; src?: string | null; large?: boolean }) {
  return (
    <div className={`relative grid place-items-center overflow-hidden rounded-lg border border-white/10 bg-gradient-to-br from-[#152018] via-[#23322a] to-[#11100d] ${large ? "min-h-[420px]" : "aspect-[4/3]"}`}>
      {src ? (
        <>
          {/* Product images can be Supabase URLs or local public placeholders. */}
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={src} alt={name} className="absolute inset-0 size-full object-contain p-3 drop-shadow-[0_16px_28px_rgba(0,0,0,0.5)]" />
        </>
      ) : (
        <>
