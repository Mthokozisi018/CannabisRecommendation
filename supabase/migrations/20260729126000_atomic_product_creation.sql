...
begin
  ...
  values (
    ...
    v_store_id,
    'success',
    jsonb_build_object('category', p_category, 'subcategory', p_subcategory, 'initialStock', p_initial_stock)
  );
...
comment on function public.create_product_with_inventory(uuid, text, text, text, text, numeric, text, integer, numeric, numeric, integer, integer) is
  'Creates a store-scoped product, stock row, initial movement, and audit record in one transaction.';
